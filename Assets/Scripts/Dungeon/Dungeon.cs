    ...
    public class Dungeon : MonoBehaviour
    {
        ...
        void MovePlayer()
        {
            ...
            if (player)
            {
                ...
                control.InitializePlayer( start.ToWorldPosition());
                control.ResetTurn();
        ...
        IEnumerator CreateTiles()
        {
            ...
            foreach (var pair in map)
            {
                if (pair.Key.x >= 0 && pair.Key.y >= 0)
                {
                    ...
                    Tile inst = Instantiate(tiles[choice], pair.Key.ToWorldPosition(), rotation);
                    ...
                    if (pair.Value > 1 && pair.Key!=start && pair.Key!=finish && Vector2.Distance(start,pair.Key)>4)
                    {
                        GameObject go = Instantiate(enemyDrops.GetDrop(level),
                                                    pair.Key.ToWorldPosition(), Quaternion.identity);
                        go.transform.Rotate(0,Random.Range(0,6)*60,0);
                        go.transform.SetParent(transform);
                        if (go.TryGetComponent<PersonalStats>(out PersonalStats stats))
                        {
                            stats.SetLevel(Mathf.Max(Random.Range(level, Mathf.Min(level+5, (int)(level*1.25f))),1));
                        }
            ...
            for(int x=startX-10;x<endx+10;x++)
            {
                for (int y = startY-10; y < endy + 10; y++)
                {
                    ...
                    if (closeEnough)
                    {
                        ...
                        {
                            GameObject inst = Instantiate(wall, new Vector2Int(x, y).ToWorldPosition(), Quaternion.identity);
                            inst.transform.SetParent(transform);
                            if(inst.TryGetComponent(out Wall w))
                            {
                                for (int i = 0; i < 6; i++)
                                {
                                    if (map.ContainsKey(new Vector2Int(x, y).Adjacent(i)))
                                    {
                                        w.ActivateWall(i);
                                    }
                                }
